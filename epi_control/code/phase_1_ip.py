...
runtimes_path = "runtimes/phase_1_ip.txt"
...
for i in range(1, 11):
    for graph_num in range(i, graphs + 1, 10):
        ...
        x = m.addVars(len(vertices), lb=0.0, ub=1.0, vtype=GRB.BINARY, name="x")
        y = m.addVars(len(vertices), instances, lb=0.0, ub=1.0, vtype=GRB.BINARY, name="y")
        ...
        m.write("results/phase_1_ip/5/" + str(graph_num) + ".sol")
        m.write("results/phase_1_ip/5/" + str(graph_num) + ".json")
